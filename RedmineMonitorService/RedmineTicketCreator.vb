...
Public Class RedmineTicketCreator
    ...
    Public Async Function CreateTicketAsync(ticketData As TicketXmlData, xmlFilePath As String) As Task(Of String)
        Try
            ...
            ' Priority: use values from XML if provided, otherwise fallback to Subject prefix logic
            Dim testServerId As String
            Dim productionServerId As String

            If Not String.IsNullOrEmpty(ticketData.TestServer) Then
                ' Use value from XML directly (add "CL" prefix if not already present)
                testServerId = If(ticketData.TestServer.StartsWith("CL"), ticketData.TestServer, "CL" & ticketData.TestServer)
                productionServerId = If(ticketData.ProductionServer.StartsWith("CL"), ticketData.ProductionServer, "CL" & ticketData.ProductionServer)
            ElseIf Not String.IsNullOrEmpty(ticketData.Subject) Then
                ' Fallback: determine by Subject prefix
